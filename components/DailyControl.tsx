...
interface Props {
  ...
  readOnly?: boolean;
  privacyMode?: boolean; 
  forcedState?: AppState; // Para modo espectador
}

const DailyControl: React.FC<Props> = ({ currentDate, setCurrentDate, notify, readOnly, privacyMode, forcedState }) => {
  // Se forcedState existir (modo espectador), usa ele. Senão, usa o store.
  const storeDailyRecords = useStore(s => s.dailyRecords);
  const storeConfig = useStore(s => s.config);
  const updateState = useStore(s => s.updateState);

  const dailyRecords = forcedState ? forcedState.dailyRecords : storeDailyRecords;
  const config = forcedState ? forcedState.config : storeConfig;

  const dayRecord = dailyRecords[currentDate] || { expenses: { proxy: 0, numeros: 0 }, accounts: [] };
  const isManualMode = config.manualBonusMode === true;
  ...
  const metrics = calculateDayMetrics(dayRecord, config.valorBonus);
  ...
  }, [dayRecord, myLeaderKey, readOnly, config.userName]);
  ...
  const bonusMultiplier = isManualMode ? 1 : (config.valorBonus || 20);
    ...
    <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="xl:w-1/3 flex flex-col gap-4">
                <div className="bg-[#0c0818] rounded-2xl p-6 border border-white/10 shadow-lg relative overflow-hidden group">
                    ...
                    <div className="relative group/date">
                        ...
                        <input type="date" className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white font-bold uppercase tracking-wide focus:border-violet-500 outline-none transition-all shadow-inner text-center cursor-pointer" value={currentDate} onChange={(e) => setCurrentDate(e.target.value)} />
                    </div>
                </div>

                <div className="bg-[#0c0818] rounded-2xl p-5 border border-white/10 flex gap-4 items-center shadow-lg">
                    <div className="flex-1">
                        <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><Globe size={10} /> Custo Proxy</label>
                        <input type="number" placeholder="0.00" className="w-full bg-black/40 border border-white/10 rounded-lg py-2 px-3 text-white font-mono font-bold text-sm focus:border-rose-500 outline-none text-right transition-colors" value={dayRecord.expenses?.proxy || ''} onChange={(e) => handleExpenseChange('proxy', e.target.value)} disabled={readOnly} />
                    </div>
                    <div className="w-px h-10 bg-white/10"></div>
                    <div className="flex-1">
                        <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 flex items-center gap-1"><Smartphone size={10} /> Custo SMS</label>
                        <input type="number" placeholder="0.00" className="w-full bg-black/40 border border-white/10 rounded-lg py-2 px-3 text-white font-mono font-bold text-sm focus:border-rose-500 outline-none text-right transition-colors" value={dayRecord.expenses?.numeros || ''} onChange={(e) => handleExpenseChange('numeros', e.target.value)} disabled={readOnly} />
                    </div>
                </div>
            </div>
            ...
            <div className="xl:w-2/3 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-[#0c0818] rounded-2xl p-6 border border-white/10 flex flex-col justify-between shadow-lg relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><ArrowDownRight size={14} className="text-rose-500" /> Investimento</p>
                        <h3 className="text-3xl font-black text-white font-mono tracking-tight drop-shadow-lg">{formatVal(metrics.invest)}</h3>
                ...
                <div className="bg-[#0c0818] rounded-2xl p-6 border border-white/10 flex flex-col justify-between shadow-lg relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><ArrowUpRight size={14} className="text-indigo-500" /> Faturamento</p>
                        <h3 className="text-3xl font-black text-white font-mono tracking-tight drop-shadow-lg">{formatVal(metrics.ret)}</h3>
                        <p className="text-xs text-indigo-400/70 mt-1 font-bold">Saques + Bônus</p>
                    </div>
                </div>
                <div className={`rounded-2xl p-6 border flex flex-col justify-between shadow-lg relative overflow-hidden group transition-all ${metrics.lucro >= 0 ? 'bg-[#0a100d] border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : 'bg-[#100a0a] border-rose-500/30 shadow-[0_0_20px_rgba(244,63,94,0.1)]'}`}>
                    <div className={`absolute -right-4 -top-4 w-32 h-32 rounded-full blur-3xl transition-all opacity-30 ${metrics.lucro >= 0 ? 'bg-emerald-500' : 'bg-rose-500'}`}></div>
                    <div className="relative z-10">
                        <p className={`text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2 ${metrics.lucro >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{metrics.lucro >= 0 ? <TrendingUp size={14} /> : <AlertCircle size={14} />} Resultado Líquido</p>
                        <h3 className={`text-4xl font-black font-mono tracking-tight drop-shadow-lg ${metrics.lucro >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatVal(metrics.lucro)}</h3>
                        <p className="text-[10px] text-gray-500 mt-2 font-bold uppercase">Margem: {metrics.ret > 0 ? ((metrics.lucro / metrics.ret) * 100).toFixed(1) : 0}%</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="min-h-[500px] flex flex-col" id="tour-daily-table">
            <div className="flex justify-between items-center mb-6 px-2">
                ...
                {!readOnly && <button onClick={handleAddAccount} aria-label="Adicionar novo registro" className="gateway-btn-primary px-6 py-2.5 rounded-xl text-xs font-bold text-white flex items-center gap-2 font-mono shadow-lg hover:shadow-primary/40 transition-all"><Plus size={14} /> NOVO REGISTRO</button>}
                ...
                <table className="w-full text-sm text-left text-gray-400 border-separate border-spacing-y-3">
                    ...
                    <tbody>
                        {dayRecord.accounts.length === 0 ? (
                            <tr><td colSpan={5} className="text-center py-20 bg-white/[0.01] rounded-2xl border border-white/5 border-dashed"><div className="flex flex-col items-center justify-center opacity-50"><div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-4"><TrendingUp size={32} /></div><h4 className="text-white font-bold mb-2">Caixa Diário Vazio</h4><p className="text-gray-400 text-sm max-w-sm mb-6">Adicione um novo registro para começar a monitorar.</p></div></td></tr>
                        ) : (
                                ...
                                return (
                                    <tr key={acc.id} className="group transition-all hover:-translate-y-1 duration-300 shadow-sm hover:shadow-lg">
                                        <td className="px-6 py-4 bg-[#0c0818] border-y border-l border-white/5 rounded-l-2xl first:rounded-l-2xl">
                                            <div className="flex items-center gap-2">
                                                <input type="number" className={`gateway-input w-24 py-2 px-3 rounded-lg text-white font-mono font-bold text-right border-white/10 focus:border-white/30 bg-black/20 ${privacyMode ? 'text-transparent bg-gray-800' : ''}`} value={acc.deposito === 0 ? '' : acc.deposito} placeholder="Dep" disabled={readOnly} onChange={(e) => handleAccountChange(acc.id, 'deposito', e.target.value)} />
                                                <span className="text-gray-600">+</span>
                                                <input type="number" className={`gateway-input w-24 py-2 px-3 rounded-lg text-gray-300 font-mono font-bold text-right border-white/5 focus:border-white/20 bg-black/20 ${privacyMode ? 'text-transparent bg-gray-800' : ''}`} value={acc.redeposito === 0 ? '' : acc.redeposito} placeholder="Re-Dep" disabled={readOnly} onChange={(e) => handleAccountChange(acc.id, 'redeposito', e.target.value)} />
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 bg-[#0c0818] border-y border-white/5">
                                            <div className="relative group/input">
                                                <input type="number" className={`gateway-input w-full py-2.5 px-3 rounded-lg text-accent-cyan font-mono font-bold border border-accent-cyan/10 focus:border-accent-cyan/50 bg-accent-cyan/[0.05] transition-all text-lg ${privacyMode ? 'text-transparent bg-gray-800' : ''}`} value={acc.saque === 0 ? '' : acc.saque} placeholder={privacyMode ? '****' : "0"} disabled={readOnly} onChange={(e) => handleAccountChange(acc.id, 'saque', e.target.value)} />
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 bg-[#0c0818] border-y border-white/5">
                                            {isManualMode ? (
                                                <div className="relative flex items-center">
                                                    ...
                                                    <input type="number" className={`gateway-input w-full py-2.5 pl-8 pr-3 rounded-lg text-primary-glow font-mono font-bold border border-primary/20 focus:border-primary/60 bg-primary/[0.05] hover:bg-primary/[0.1] transition-all ${privacyMode ? 'text-transparent bg-gray-800' : ''}`} value={acc.ciclos === 0 ? '' : acc.ciclos} placeholder={privacyMode ? '****' : "0.00"} disabled={readOnly} onChange={(e) => handleAccountChange(acc.id, 'ciclos', e.target.value)} />
                                                </div>
                                            ) : (
                                                <div className="flex items-center bg-black/30 border border-white/10 rounded-lg p-1 w-full max-w-[140px]">
                                                    <input type="number" className="w-full bg-transparent text-center text-white font-mono font-bold py-1.5 focus:outline-none" value={acc.ciclos === 0 ? '' : acc.ciclos} placeholder="0" disabled={readOnly} onChange={(e) => handleAccountChange(acc.id, 'ciclos', e.target.value)} />
                                                    <div className="bg-white/5 border-l border-white/10 px-2.5 py-1.5 flex flex-col items-center justify-center min-w-[50px]">
                                                        ...
                                                        <span className="text-[10px] text-primary-glow font-mono font-bold leading-none">{config.valorBonus}</span>
                                                    </div>
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 bg-[#0c0818] border-y border-white/5 text-right">
                                            <div className={`inline-flex items-center justify-end gap-2 px-4 py-2 rounded-xl border backdrop-blur-md min-w-[140px] shadow-lg transition-all duration-500 ${lucro >= 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400 shadow-emerald-500/10' : 'bg-rose-500/10 border-rose-500/20 text-rose-400 shadow-rose-500/10'}`}>
                                                <span className="text-[10px] font-bold opacity-60 uppercase tracking-wider mr-auto">{lucro >= 0 ? <ArrowUpRight size={14}/> : <ArrowDownRight size={14}/>}</span>
                                                <span className="text-lg font-black font-mono tracking-tight">{formatVal(lucro)}</span>
                                            </div>
                                        </td>

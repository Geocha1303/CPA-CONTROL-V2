    ...
    <div className="space-y-6 max-w-[1600px] mx-auto animate-fade-in pb-10">
      ...
      <div className="flex flex-col xl:flex-row gap-6">
          ...
          <div className="flex-grow grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="gateway-card p-4 rounded-xl border-l-2 border-l-primary">
                    <span className="text-[10px] text-gray-500 uppercase font-bold font-mono block mb-1">ENTRADAS (TOTAL)</span>
                    <span className="text-xl font-bold text-white font-mono">
                        {formatarBRL(totalSaques + totalBonus)}
                    </span>
                </div>
                <div className="gateway-card p-4 rounded-xl border-l-2 border-l-accent-pink">
                    <span className="text-[10px] text-gray-500 uppercase font-bold font-mono block mb-1">SAÍDAS (TOTAL)</span>
                    <span className="text-xl font-bold text-white font-mono">
                        {formatarBRL(totalInvestido)}
                    </span>
                </div>
                 <div className="gateway-card p-4 rounded-xl border-l-2 border-l-gray-600">
                    <span className="text-[10px] text-gray-500 uppercase font-bold font-mono block mb-1">OPS COSTS</span>
                    <span className="text-xl font-bold text-white font-mono">
                ...
                <div className={`gateway-card p-4 rounded-xl border-l-2 ${totalLucroDia >= 0 ? 'border-l-accent-cyan bg-accent-cyan/5' : 'border-l-accent-pink bg-accent-pink/5'}`}>
                    <span className="text-[10px] text-gray-400 uppercase font-bold font-mono block mb-1">NET PROFIT</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className="gateway-card p-4 rounded-xl flex items-center justify-between">
            <span className="text-sm text-gray-300 font-bold font-mono">PROXY COSTS</span>
         ...
         <div className="gateway-card p-4 rounded-xl flex items-center justify-between">
            <span className="text-sm text-gray-300 font-bold font-mono">SMS COSTS</span>
            <div className="relative w-40">
                <input type="number" className="gateway-input w-full pl-3 pr-3 py-2 text-right rounded font-mono font-bold text-sm"
      ...
      <div className="gateway-card rounded-xl overflow-hidden min-h-[500px] flex flex-col">
        ...
        <div className="overflow-x-auto flex-1">
            <table className="w-full text-sm text-left text-gray-400">
                <thead className="text-[10px] text-gray-500 uppercase bg-black/40 border-b border-white/5 font-mono">
                    <tr>
                        <th className="px-6 py-4 font-bold tracking-widest">DEPOSIT</th>
                        <th className="px-6 py-4 font-bold tracking-widest">RE-DEP</th>
                        <th className="px-6 py-4 font-bold tracking-widest text-accent-cyan">WITHDRAW</th>
                        <th className="px-6 py-4 font-bold tracking-widest text-primary-glow">CYCLES</th>
                        <th className="px-6 py-4 font-bold tracking-widest text-right">RESULT</th>
                ...
                <tbody className="divide-y divide-white/5">
                    {dayRecord.accounts.length === 0 ? (
                        <tr>
                            <td colSpan={6} className="text-center py-20 text-gray-600 font-mono">
                                NO_DATA_AVAILABLE
                            </td>
                        </tr>
                    ) : (
                                ...
                                <tr key={acc.id} className="hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <td className="px-6 py-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] text-gray-600 font-mono">x{state.config.valorBonus}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-3 text-center">
                                        <button onClick={() => handleDeleteAccount(acc.id)} className="p-2 rounded hover:bg-white/10 text-gray-600 hover:text-accent-pink transition-colors">
                                            <Trash2 size={16} />
                                        </button>
                                    </td>
                                </tr>
